...
for k in range(len(time1)):
    ax.plot(x, time1[k] * np.ones(len(x)), T1[k], color = 'k')
...
for m in range(len(time3)):
    ...
    ax.plot(xt, time_t, Tt, color = 'k')
    ax.plot(xt[:np.where(np.isclose(Tt, Tf))[0][0] + 1], time_t[:np.where(np.isclose(Tt, Tf))[0][0] + 1], Tt[:np.where(np.isclose(Tt, Tf))[0][0] + 1], color = 'g')
    ...
    for r in range(len(idx_x_lb_t)):
        ax.plot(xt[idx_x_ub_t[r]:idx_x_lb_t[r] + 1], time_t[idx_x_ub_t[r]:idx_x_lb_t[r] + 1], Tt[idx_x_ub_t[r]:idx_x_lb_t[r] + 1], color = 'b')
