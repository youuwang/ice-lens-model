...
void FDM_icelens::solve(Ice_model* ptr) {
    ...
    for (int i = 1; i < this->N - 1; i++) {
        if (i == idx_xl) {
            ...
            ptr->rho_i * ptr->L * ptr->vl * (1 - ptr->phi * si_back) * (ptr->x[idx_xl + 1] - xl) / ptr->dx) / ptr->rho_c_eff[i];
        }
        else if (i == idx_xl + 1) {
            ptr->T[i] = T_temp[i] + this->dt * (((ptr->kappa[i + 1] + ptr->kappa[i]) / 2 * (T_temp[i + 1] - T_temp[i]) - (ptr->kappa[i - 1] + ptr->kappa[i]) / 2 * (T_temp[i] - T_temp[i - 1])) / pow(this->dx, 2) + \
            ptr->rho_i * ptr->L * ptr->vl * (1 - ptr->phi * si_back) * (xl - ptr->x[idx_xl]) / ptr->dx) / ptr->rho_c_eff[i];        
        }
        else {
